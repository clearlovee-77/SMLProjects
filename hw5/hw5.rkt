...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [[call? e]
         [let* [[s1 [eval-under-env [call-funexp e] env]]
               [s2 [eval-under-env [call-actual e] env]]]
           [if [not [closure? s1]]
               [error "function is not found"]
               [let* [[env [closure-env s1]]
                      [s3 [closure-fun s1]]
                      [s4 [fun-nameopt s3]]
                      [ex-env1 [cons [cons [fun-formal s3] s2] env]]
                      [ex-env2 [if [equal? s4 #f]
                                   ex-env1
                                   [cons [cons s4 s1] ex-env1]]]]
                 [eval-under-env [fun-body s3] ex-env2]]]]]
        [[apair? e]
         [let [[s1 [eval-under-env [apair-e1 e] env]]
               [s2 [eval-under-env [apair-e2 e] env]]]
           [apair s1 s2]]]
        [[fst? e]
         [let [[v [eval-under-env [fst-e e] env]]]
           [if [apair? v]
               [apair-e1 v]
               [error "bad apair in fst"]]]]
        [[snd? e]
         [let [[v [eval-under-env [snd-e e] env]]]
           [if [apair? v]
               [apair-e2 v]
               [error "bad apair in snd"]]]]
        [[isaunit? e]
...
(define mupl-map
  [fun "map1" "f"
       [fun "map2" "mlist"
            [ifgreater [isaunit [var "mlist"]] [int 0]
                       [aunit]
                       [apair [call [var "f"] [fst [var "mlist"]]]
                              [call [var "map2"] [snd [var "mlist"]]]]]]])
